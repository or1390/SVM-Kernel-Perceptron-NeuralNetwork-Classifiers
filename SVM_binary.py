...
class support_vector_machine():
    ...
    def __init__(self, lambda_rate, num_of_iterations):
        ...
        self.weights = []
    '''
    this function is used to train the model using the Pegasus paper for 
    ...
    def train(self, X_train, y_train):
        ...
        for i in tqdm(range (self.num_of_iterations)):
            random_index = np.random.choice(n_samples, 1)[0]  #generate a random index to determine the current instance
            current_step_size = 1 / (self.lambda_rate * (i + 1)) #step size for each iteration
        ...
        return (self.weights)
    '''
    For each instance in the training method, it predicts the label
    '''
